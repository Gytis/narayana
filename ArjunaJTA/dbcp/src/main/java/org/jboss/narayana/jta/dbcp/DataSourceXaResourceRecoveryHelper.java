...
public class DataSourceXaResourceRecoveryHelper
    ...
    public DataSourceXaResourceRecoveryHelper(XADataSource xaDataSource, String user,
            String password) {
        Objects.requireNonNull(xaDataSource, "XADataSource must not be null");
    ...
    private boolean connect() {
        if (this.delegate == null) {
            try {
                ...
                jtaLogger.logger.warn("Failed to create connection", ex);
                return false;
            }
        }
        return true;
    ...
    private void disconnect() throws XAException {
        try {
            ...
            jtaLogger.logger.warn("Failed to close connection", e);
        } finally {
