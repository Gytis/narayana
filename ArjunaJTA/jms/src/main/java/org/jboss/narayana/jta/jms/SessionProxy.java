...
public class SessionProxy implements Session {
    ...
    private static final Logger LOGGER =
            Logger.getLogger(SessionProxy.class.getName());
    ...
    public void close() throws JMSException {
        if (transactionHelper.isTransactionAvailable()) {
            ...
            if (LOGGER.isLoggable(Level.FINE)) {
                LOGGER.fine("Delisted " + xaSession
                        + " XA resource from the transaction");
            }

            Synchronization synchronization =
                    new SessionClosingSynchronization(xaSession);
            transactionHelper.registerSynchronization(synchronization);

            if (LOGGER.isLoggable(Level.FINE)) {
                LOGGER.fine("Registered synchronization to close the session: "
                        + synchronization);
            }
    ...
    public void commit() throws JMSException {
        // TODO maybe throw exception straight away since this should not happen?
        xaSession.commit();
    }
    ...
    @Override
    public TopicSubscriber createDurableSubscriber(Topic topic, String s,
            String s1, boolean b) throws JMSException {
