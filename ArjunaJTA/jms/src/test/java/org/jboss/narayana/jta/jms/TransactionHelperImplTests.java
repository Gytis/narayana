...
public class TransactionHelperImplTests {
    ...
    public void transactionShouldBeAvailable() throws Exception {
        when(transactionManagerMock.getStatus())
                .thenReturn(Status.STATUS_ACTIVE);
    ...
    public void shouldRegisterSynchronization() throws Exception {
        when(transactionManagerMock.getTransaction())
        ...
        verify(transactionMock, times(1))
                .registerSynchronization(synchronizationMock);
    }
    ...
    public void shouldFailToRegisterSynchronization() throws Exception {
        when(transactionManagerMock.getTransaction())
                .thenReturn(transactionMock);
        doThrow(new IllegalStateException()).when(transactionMock)
    ...
    public void shouldFailToEnlistResource() throws Exception {
        when(transactionManagerMock.getTransaction())
                .thenReturn(transactionMock);
        when(transactionMock.enlistResource(any(XAResource.class)))
    ...
    public void shouldDelistResource() throws Exception {
        ...
        verify(transactionManagerMock, times(1)).getTransaction();
        verify(transactionMock, times(1))
                .delistResource(xaResourceMock, XAResource.TMSUCCESS);
    }
    ...
    public void shouldFailToDeslitResource() throws Exception {
        when(transactionManagerMock.getTransaction())
                .thenReturn(transactionMock);
        when(transactionMock.delistResource(any(XAResource.class), anyInt()))
                .thenThrow(new IllegalStateException());
